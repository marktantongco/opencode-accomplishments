...
name: mcp-builder
description: >
  Build MCP servers with TypeScript + Python, full lifecycle planning. Use when use when building mcp servers or tool integrations
  Triggers on keywords: mcp, typescript, python, servers.
...
Build MCP servers with TypeScript + Python, full lifecycle from scaffold to deploy. Generates server scaffolding, tool definitions, resource handlers, and transport configuration following the MCP specification. This skill provides structured workflows and production-ready patterns that ensure code quality, maintainability, and developer experience.
...
**Input:** "Use when building MCP servers or tool integrations"
...
### Approach
Use the established mcp builder pattern to implement the feature with minimal coupling and maximum testability.
